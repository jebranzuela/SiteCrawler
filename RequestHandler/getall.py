...
with open('links_with_error.csv', 'r') as f:
    reader = csv.reader(f)
    links = list(reader)
...
for link in links:
    try:
        r = requests.get(link[0],headers=headers,allow_redirects=True, verify=False)
        ...
        if r.history and (r.status_code >= 200 and r.status_code <= 310):
            ...
            print(link[0] + ',' + link[1] + ',' + str(r.status_code), file=open("external_success_2.txt", "a"))
            ...
            print(link[0] + ',' + link[1] + ',' + str(r.status_code), file=open("links_with_error_2.txt", "a"))
            print(link[0] + ',' + link[1] + ',' + str(r.status_code), file=open("external_error_2.txt", "a"))
        ...
        print(link[0] + ',' + link[1] + ',0', file=open("links_with_error_2.txt", "a"))
        print(link[0] + ',' + link[1] + ',0', file=open("external_error_2.txt", "a"))
